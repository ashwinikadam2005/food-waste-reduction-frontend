...
const DonorProfile = () => {
  ...
  useEffect(() => {
    const fetchDonorProfile = async () => {
      try {
        const url = donorId
          ? `http://localhost:5001/ratingmodel/donor-profile/${donorId}`
          : 'http://localhost:5001/donor-profile/me';
        ...
        {donorProfile.ratings.length > 0 ? (
          <div>
            {donorProfile.ratings.map((r, i) => (
              <div key={i} className="review-item">
                <p className="receiver-name">Receiver: {r.receiver_name}</p>
                <p className="food-name"><strong>Food:</strong> {r.food_name}</p>

                <p className="rating-stars">{renderStars(r.rating)}</p>
                {r.review && <p className="review-text">“{r.review}”</p>}
                <p className="timestamp">{new Date(r.createdAt).toLocaleDateString()}</p>
              </div>
            ))}
          </div>
        ) : (
